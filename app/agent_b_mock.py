...
def serve(api_port: int, idle_timeout: float | None = None) -> None:
    ...
    print(f"[agent_b] listening on AXL bridge :{api_port} as pubkey {pk[:16]}...")
    ...
    while True:
        try:
            ...
            if idle_timeout is not None and time.monotonic() - started > idle_timeout:
                print(f"[agent_b] idle timeout after {idle_timeout}s, shutting down")
                return
            continue
        try:
            spec = json.loads(body.decode("utf-8"))
        except Exception as e:
            print(f"[agent_b] dropped non-JSON message from {sender[:16]}: {e}")
            continue
        print(f"[agent_b] job from {sender[:16]}: {spec}")
        result = execute(spec)
        result["job_id"] = spec.get("job_id")
        send_bytes(sender, json.dumps(result).encode("utf-8"), api_port)
        print(f"[agent_b] replied: {result}")
